...
> **为什么不在镜像里装 perf？**
> `linux-tools-generic` 装的是"构建镜像时那台机器的内核版本对应的 perf"，如果实际运行的宿主机内核版本不一致，perf 会直接报错或解析失败。本项目改为**运行时挂载宿主机真实的 perf 二进制**到容器里（见 `docker-compose.yml` 的 `drop_agent` / `analysis` 服务），这样无论在哪台机器上跑，perf 版本永远和宿主机内核匹配。**唯一前提是宿主机自己要装好对应内核版本的 `linux-tools-generic`。**
...
> **启动前必须先解析出本机真实 perf 路径，写入 `.env` 文件（只需做一次，docker compose 每次启动会自动读取）：**
> ```bash
> echo "HOST_PERF_PATH=$(readlink -f /usr/lib/linux-tools/$(uname -r)/perf)" > .env
> cat .env   # 确认输出的是一个真实存在的 ELF 文件路径，例如：
>            # HOST_PERF_PATH=/usr/lib/linux-hwe-6.8-tools-6.8.0-124/perf
> ```
> **注意：不要用 `which perf` 或 `readlink -f $(which perf)` 来解析！**
> `which perf` 拿到的 `/usr/bin/perf` 在 Ubuntu 上**不是软链接，而是一段检测脚本**（内部依赖
> `lsb_release` 判断发行版再去找真实二进制），容器里没有 `lsb_release` 会导致这段脚本直接报错。
> 真正的软链接入口是 `/usr/lib/linux-tools/<内核版本>/perf`，要从这里开始用 `readlink -f`
> 解析，才能拿到链条最末端的真实 ELF 二进制路径。
>
> **如果用 `export HOST_PERF_PATH=...` 而不是写入 `.env`：** 一定要确认当前终端会话里没有
> 残留的旧值（`echo $HOST_PERF_PATH` 检查一下），因为 shell 里 `export` 过的环境变量优先级
> 高于 `.env` 文件，旧值不清除的话 `.env` 里改对了也不会生效。**推荐统一用 `.env` 文件**，
> 避免这个坑，导师换终端、换会话都不需要重新设置。
...
```bash
echo "HOST_PERF_PATH=$(readlink -f /usr/lib/linux-tools/$(uname -r)/perf)" > .env   # 见上方"环境要求"说明，必须先执行
cat .env   # 确认路径解析成功，不是空的
