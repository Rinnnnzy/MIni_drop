...
> **启动前必须先解析出本机真实 perf 路径（每次开新终端都要重新 export 一次）：**
> ```bash
> export HOST_PERF_PATH=$(readlink -f $(which perf))
> echo $HOST_PERF_PATH   # 确认输出了一个真实存在的文件路径，不是空的
> ```
> `which perf` 拿到的通常是 `/usr/bin/perf`，但它在大多数发行版里是软链接，最终指向类似
> `/usr/lib/linux-tools-<内核版本>/perf` 的真实二进制。`readlink -f` 会自动把链接走到底，
> 不管导师机器内核版本是多少、目录命名是 `linux-tools` 还是 `linux-hwe-tools`，这条命令
> 都能解析出正确路径，**不需要手动改 `docker-compose.yml`**。
...
```bash
export HOST_PERF_PATH=$(readlink -f $(which perf))   # 见上方"环境要求"说明，必须先执行
